# =========================================================
# VISUALIZADOR STM32 + IZHIKEVICH — SENSOR 5x5
# =========================================================
#
# Análogo 5x5 do touch_sensor4x4.py: lê a serial do STM32, plota os mesmos
# gráficos (heatmap, raster RA/SA, sinal escalar e neurônio pós) e RETRANSMITE
# por UDP para o touch_receiver_node do touch_pack (porta 8081, payload '<If').
#
# Diferenças em relação ao 4x4:
#   • Grade 5x5 (25 taxels) em vez de 4x4 (16).
#   • O firmware 5x5 NÃO envia a linha TOTAL/Ifinal. O sinal escalar de ~1 kHz
#     que vai pelo UDP é sintetizado por FRAME: ao chegar o ÚLTIMO taxel da
#     varredura (idx == 24) o heatmap está completo e emitimos a MÉDIA das
#     tensões (faixa 0..VREF) como "ativação" daquele frame. É o mesmo papel do
#     I_final no 4x4 — um escalar por frame, retransmitido e plotado.
#
# A GUI do touch_pack (sensor:='5') faz exatamente isto embutido (touch_source.py);
# este script é o plotter standalone equivalente, útil no modo no_gui ou em
# outro PC. O streaming permanece em 1 ms / 1 kHz (um valor por frame do STM32).
#
# Gravação opcional (--record PLANILHA.csv): uma linha por frame, datada pelo
# RELÓGIO DO FIRMWARE (micros()/1e6), com as 25 tensões e a ativação média.
